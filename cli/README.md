...
pyghidra-mcp-cli decompile --program my_binary.dylib main

# Search for symbols
pyghidra-mcp-cli search symbols --program my_binary.so malloc -l 20
```
...
Commands:
  ...
  list         List programs, imports, exports
...
```bash
pyghidra-mcp-cli decompile --program <program_name> <function_name_or_address>
```
...
**Search for symbols:**
```bash
pyghidra-mcp-cli search symbols --program <program_name> <query> [options]
...
```bash
pyghidra-mcp-cli search code --program <program_name> <query> [options]
...
**Search for strings:**
```bash
pyghidra-mcp-cli search strings --program <program_name> <query> [options]
...
**List imports:**
```bash
pyghidra-mcp-cli list imports --program <program_name> [options]
...
**List exports:**
```bash
pyghidra-mcp-cli list exports --program <program_name> [options]
...
```bash
pyghidra-mcp-cli xref --program <program_name> <name_or_address>
```
...
```bash
pyghidra-mcp-cli read --program <program_name> <address> [options]
...
```bash
pyghidra-mcp-cli callgraph --program <program_name> <function_name> [options]
...
```bash
pyghidra-mcp-cli delete --program <program_name>
```
...
```bash
pyghidra-mcp-cli metadata --program <program_name>
```
...
pyghidra-mcp-cli search symbols --program malware_sample "crypto" -l 20

# Decompile a key function
pyghidra-mcp-cli decompile --program malware_sample crypto_init

# List all imports to understand dependencies
pyghidra-mcp-cli list imports --program malware_sample -l 50

# Check cross-references
pyghidra-mcp-cli xref --program malware_sample 0x401000

# Generate a call graph
pyghidra-mcp-cli callgraph --program malware_sample main -d calling

# Show binary metadata
pyghidra-mcp-cli metadata --program malware_sample
```
...
pyghidra-mcp-cli search symbols --program test_binary function -o 0 -l 10

# Search code with semantic mode
pyghidra-mcp-cli search code --program test_binary "function_one" -m semantic -l 5

# Search strings
pyghidra-mcp-cli search strings --program test_binary "Hello" -l 10

# List imports with regex filter
pyghidra-mcp-cli list imports --program test_binary -q ".*printf.*" -l 10

# List exports with regex filter
pyghidra-mcp-cli list exports --program test_binary -q ".*function.*" -l 10

# Decompile function by name
pyghidra-mcp-cli decompile --program test_binary main

# Decompile function by address
pyghidra-mcp-cli decompile --program test_binary 0x401000

# List cross-references to symbol
pyghidra-mcp-cli xref --program test_binary function_one

# List cross-references to address
pyghidra-mcp-cli xref --program test_binary 0x401000

# Read bytes from memory
pyghidra-mcp-cli read --program test_binary 0x100000 -s 32

# Generate call graph (functions called by main)
pyghidra-mcp-cli callgraph --program test_binary main -d calling -t flow

# Generate call graph (functions that call main)
pyghidra-mcp-cli callgraph --program test_binary main -d called -t flow_ends

# Import new binary
pyghidra-mcp-cli import /path/to/new_binary

# Delete binary from project
pyghidra-mcp-cli delete --program test_binary

# Show binary metadata
pyghidra-mcp-cli metadata --program test_binary
```
...
```bash
# Text format (default)
pyghidra-mcp-cli search symbols --program my_binary.so malloc -l 5

# JSON format for automation
pyghidra-mcp-cli --format json search symbols --program my_binary.exe malloc | jq '.symbols[0].name'

# Table format for readable output
pyghidra-mcp-cli --format table list imports --program my_binary.exe
```
...
pyghidra-mcp-cli search code --program test_binary "function_one" -m semantic --full-code -l 3

# Literal code search with custom preview
pyghidra-mcp-cli search code --program test_binary "printf" -m literal -p 100 -l 5

# Code search with similarity threshold
pyghidra-mcp-cli search code --program test_binary "error handling" -t 0.7 -l 10

# Symbol search with offset for pagination
pyghidra-mcp-cli search symbols --program test_binary "func" -o 10 -l 20
```
...
pyghidra-mcp-cli callgraph --program test_binary main

# Reverse call graph (functions that call main)
pyghidra-mcp-cli callgraph --program test_binary main -d called

# Call graph with flow_ends display type
pyghidra-mcp-cli callgraph --program test_binary main -t flow_ends

# Call graph with custom condensation settings
pyghidra-mcp-cli callgraph --program test_binary main --condense-threshold 100 --top-layers 5 --bottom-layers 5

# Call graph with time limit
pyghidra-mcp-cli callgraph --program test_binary main --max-run-time 300
```
...
pyghidra-mcp-cli --host 192.168.1.100 --port 8000 list programs
```
...
Then use the exact binary name from the list with the `--program` flag.
...
| Task | Command |
|------|---------|
| List programs | `pyghidra-mcp-cli list programs` |
| Search symbols | `pyghidra-mcp-cli search symbols --program BINARY QUERY -l LIMIT` |
| Decompile function | `pyghidra-mcp-cli decompile --program BINARY FUNCTION` |
| List imports | `pyghidra-mcp-cli list imports --program BINARY -l LIMIT` |
| List exports | `pyghidra-mcp-cli list exports --program BINARY -l LIMIT` |
| Generate call graph | `pyghidra-mcp-cli callgraph --program BINARY FUNCTION -d DIRECTION` |
| Show metadata | `pyghidra-mcp-cli metadata --program BINARY` |
| Delete binary | `pyghidra-mcp-cli delete --program BINARY` |
...
pyghidra-mcp-cli list programs
pyghidra-mcp-cli search symbols --program my_binary.exe "main" -l 5
pyghidra-mcp-cli decompile --program my_binary.exe main
...
pyghidra-mcp-cli list programs
pyghidra-mcp-cli decompile --program test_binary main
pyghidra-mcp-cli search symbols --program test_binary "function" -l 10
pyghidra-mcp-cli search code --program test_binary "function_one" -l 5 --full-code
pyghidra-mcp-cli search strings --program test_binary "Hello" -l 10
pyghidra-mcp-cli list imports --program test_binary -q ".*printf.*" -l 10
pyghidra-mcp-cli list exports --program test_binary -q ".*function.*" -l 10
pyghidra-mcp-cli xref --program test_binary function_one
pyghidra-mcp-cli read --program test_binary 100000 -s 32
pyghidra-mcp-cli callgraph --program test_binary main -d calling -t flow
pyghidra-mcp-cli metadata --program test_binary
```
