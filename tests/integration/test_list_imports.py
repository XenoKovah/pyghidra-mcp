...
async def test_list_imports(server_params_shared_object, test_shared_object):
    ...
    async with stdio_client(server_params_shared_object) as (read, write):
        async with ClientSession(read, write) as session:
            ...
            # Test without params
            response = await session.call_tool("list_imports", {"program_name": program_name})
            ...
            response = await session.call_tool(
                "list_imports", {"program_name": program_name, "query": "printf", "limit": 1}
            )
            import_infos = ImportInfos.model_validate_json(response.content[0].text)
            ...
            # Test offset
            response = await session.call_tool(
                "list_imports", {"program_name": program_name, "offset": 1, "limit": 1}
            )
            import_infos = ImportInfos.model_validate_json(response.content[0].text)
            assert len(import_infos.imports) == 1
            ...
            # Test query
            response = await session.call_tool(
                "list_imports", {"program_name": program_name, "query": "printf"}
            )
            import_infos = ImportInfos.model_validate_json(response.content[0].text)
            assert len(import_infos.imports) >= 1
            ...
            response = await session.call_tool(
                "list_imports", {"program_name": program_name, "query": "non_existent_import"}
            )
            import_infos = ImportInfos.model_validate_json(response.content[0].text)
            assert len(import_infos.imports) == 0
